...
def central_difference(f, *vals, arg=0, epsilon=1e-6):
    ...
    """
    val_upper = f(*(vals[i] if i != arg else vals[i] + epsilon for i in range(len(vals))))
    val_lower = f(*(vals[i] if i != arg else vals[i] - epsilon for i in range(len(vals))))
    return (val_upper - val_lower) / (2 * epsilon)
